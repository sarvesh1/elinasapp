...
exports.api = function api(token, refresh, apiVersion) {
    ...
    return {
        ...
        updateStatus: function updateStatus(id, fields, callback, error) {
                var  options   =  {
                        oauth:   oauth,
                        refresh:refresh,
                        path:   '/v'  +   apiVersion  +   '/chatter/feeds/news/'  +   id  +   '/feed-items' + '?' + querystring.stringify(fields),
                        callback:callback,
                        error:error,
			method: 'POST',
			data: {}
